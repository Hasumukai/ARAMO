...
#define MAX 1000000
#define SRATE 48000 //sampling rate [Hz]
#define WINDOW_LENGTH 48000
...
//		calc_fft(&Y[i*SHIFT_LENGTH],in,out,start_freq,end_freq);
		calc_fft(Y,in,out,start_freq,end_freq);
		printf("\n");
//	}

	if(ret=fseek(fp, i*SRATE*2, SEEK_SET)){
			fprintf(stderr, "%d:fseek error.\n",ret);
			exit(-1);
  }
	}
