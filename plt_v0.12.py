...
#		measure_freq_high=plt_sf+start_freq+350
		measure_freq_low=plt_sf+start_freq+200
		measure_freq_high=plt_sf+start_freq+400
#		measure_freq_low=550
#		measure_freq_high=650
#		ax.vlines(1, measure_freq_low-start_freq, measure_freq_high-start_freq, color='g')
		ax.vlines(600, measure_freq_low-start_freq, measure_freq_high-start_freq, color='g')
...
#				yvn=In*sum(zn[zn>self.ZMIN])/(measure_upper-measure_lower)
				yvn=sum(zn[zn>self.ZMIN])/(measure_upper-measure_lower)
				yv.append(yvn)
			return yv
			#print(yv)
...
#		THRESHOLD=1500
		...
		ax_hist.set_ylim([0,THRESHOLD*3+10])
		ax_hist.patch.set_alpha(0)  # subplotの背景透明度
		ax_hist.grid(axis="y", color="white")
		plt.yticks(np.arange(0,THRESHOLD*3+10,THRESHOLD))
...
FreqWidth=600
FreqShift=FreqWidth-100
#for plt_sf in range(0,end_freq-start_freq,FreqWidth):
for plt_sf in range(0,end_freq-start_freq,FreqShift):
#	if plt_sf+FreqWidth > end_freq:
	if plt_sf+start_freq+FreqWidth > end_freq:
		plt_sf=(end_freq-start_freq)-FreqWidth
	print(plt_sf+start_freq)
	#plt_sf=0	#300
